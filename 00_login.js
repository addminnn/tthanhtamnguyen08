...
// ===== default class (single-class mode) =====
// Run system with ONE fixed class (no class selection). All student sessions use this.
...
function getSession(){
  try {
    ...
    if(s.role === "student") {
      ...
      if(!st) return null;
      return { role:"student", id: st.id, name: st.name || "", class: DEFAULT_CLASS };
    }
    if(s.role === "teacher") {
...
function showStudentApp(sess){
  ...
  window.__USER = sess;
  // Ensure iframe (editor_v2) can read full student info (class/name) from SESSION_KEY
  // single-class: always store DEFAULT_CLASS in session
  try{ setSession({role:"student", id: sess.id, name: sess.name||"", class: DEFAULT_CLASS}); }catch(e){}
...
function loginTry(){
  ...
  if(loginRole === "student"){
    ...
    // single-class: always store DEFAULT_CLASS in session
    setSession({role:"student", id: st.id, name: st.name || "", class: DEFAULT_CLASS});
    location.reload();
    return;
  }
