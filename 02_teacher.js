...
(function(){
  ...
  function initFirebaseSync(){
    ...
    // Ngan hang de bai tu tao realtime (de HS mo bai custom)
    try{
      if(typeof FB.listenBank === "function"){
        FB.listenBank((map)=>{
          __fbBankToLocal(map);
  ...
  function seedRosterIfEmpty(){
    try{
      ...
      let list = [];
      // NOTE:
      // teacher_dashboard.html chạy trong iframe của index.html.
      // Vì vậy, dữ liệu STUDENTS / hàm getStudentList thường nằm ở window.parent.
      // Nếu không seed được roster, GV sẽ thấy "Chưa có học sinh" dù app có sẵn danh sách HS.
      ...
      if(Array.isArray(list) && list.length){
        ...
        // ✅ If Firebase is enabled, also push seeded roster to Firestore once.
        // Otherwise: GV thấy có danh sách HS nhưng Firestore trống (máy khác không thấy).
        try{
          const FB = (window.py10Firebase && window.py10Firebase.enabled) ? window.py10Firebase : null;
          // Seed roster to Firestore *at least once per browser*, but also retry if Firestore is still empty.
          const seededFlag = "py10:fb:rosterSeeded";
          if(FB){
            const doSeed = async () => {
              try{
                // If we have never seeded OR Firestore currently has no students yet -> seed now
            ...
            // Run async seeding without blocking UI
            try{ doSeed(); }catch(e){}
          }
        }catch(e){}
        return seeded;
  ...
  function fbEnabled(){ return !!(window.py10Firebase && window.py10Firebase.enabled); }
  function getSyncedProgress(studentId){
    const sid = String(studentId||'').trim();
    if(!sid) return { passed:{} };
    if(fbEnabled()){
      const d = (window.py10Firebase._progressMap || {})[sid] || null;
...
// ===== Data =====
  ...
  function setView(name){
    // 🔒 Disabled per request: hide "Ngân hàng câu hỏi" entirely.
  ...
  function renderAssign(){
    ...
    view.innerHTML = `
      <div class="tCard">
        ...
        <div class="muted">Giao bài hệ thống hoặc đề riêng. Bài GV giao sẽ hiển thị trong danh sách bài của học sinh <b>sau 5 bài có sẵn</b> (bắt đầu từ <b>Bài 6</b>). Học sinh cần <b>PASS đủ 5 bài đầu</b> mới mở được bài GV.</div>
    ...
    $("aGoBank").onclick = ()=>{
      ...
      modal(`
        ...
        <div class="muted" style="margin-top:10px; line-height:1.5;">
          ...
          Học sinh sẽ thấy đề này ở danh sách bài <b>sau 5 bài có sẵn</b> (bắt đầu từ <b>Bài 6</b>).
        </div>
      `);
